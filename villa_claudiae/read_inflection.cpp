...
static string get_translate_conjugation(const string& s)
{
    static map<string,string> trans{ { "1.", "first_c" },
                                     { "2.", "second_c" },
                                     { "3.", "third_c" },
    ...
    try {
        ...
        cerr << e.what() << "\n";
        cerr << "Problem with verb conjugation: '" << s << "'\n";
        exit(1);
    }
}
...
static string get_translate_declension(const string& s)
{
    static map<string,string> trans{ { "1.", "first_d" },
                                     { "1./2.", "first_or_second_d" },
                                     { "2.", "second_d" },
    ...
    try {
        ...
        cerr << e.what() << "\n";
        cerr << "Problem with declension: '" << s << "'\n";
        exit(1);
    }
}
...
static string get_translate_stem(const string& s)
{
    static map<string,string> trans{ { "a-stamme", "a" },
                                     { "b-stamme", "b" },
                                     { "c-stamme", "c" },
...
void add_conjugation(const string& w, const string& s)
{
    auto [it, success] = conj_map.insert(pair{w, get_translate_conjugation(s)});

    if (!success)
        assert(it->second == get_translate_conjugation(s));
}    

void add_adj_declension(const string& w, const string& s, const string& stem)
{
    auto [it, success] = adj_decl_map.insert(pair{w, get_translate_declension(s)});

    if (!success)
        assert(it->second == get_translate_declension(s));

    tie(it, success) = adj_stem_map.insert(pair{w, get_translate_stem(stem)});

    if (!success)
        assert(it->second == get_translate_stem(stem));
}

void add_noun_declension(const string& w, const string& s, const string& stem)
{
    auto [it, success] = noun_decl_map.insert(pair{w, get_translate_declension(s)});

    if (!success)
        assert(it->second == get_translate_declension(s));

    tie(it, success) = noun_stem_map.insert(pair{w, get_translate_stem(stem)});
